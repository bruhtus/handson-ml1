...
def main(data):
    ...
    #print(
    #    f'Income categories proportions full housing dataset:\n{income_cat_proportions(housing)}\n'
    #)
    #print(
    #    f'Stratum train set income categories proportions:\n{income_cat_proportions(strat_train_set)}\n'
    #)
    #print(
    #    f'Stratum test set income categories proportions:\n{income_cat_proportions(strat_test_set)}\n'
    #)

    train_set, test_set = train_test_split(
        housing, test_size=0.2,
        random_state=42)  #add income_cat to test_set and train_set
    #compare_props(housing, strat_test_set, test_set)

    for set_ in (strat_train_set, strat_test_set):
        set_.drop('income_cat', axis=1,
                  inplace=True)  #remove income categories
    ...
    #scatter_plot(housing_strat_train, 'median_income', 'median_house_value',
    #              0.1)

    #look correlations rooms per household, bedrooms per room, and population per household
    housing_strat_train['rooms_per_household'] = housing_strat_train[
        'total_rooms'] / housing_strat_train['households']
    housing_strat_train['bedrooms_per_room'] = housing_strat_train[
        'total_bedrooms'] / housing_strat_train['total_rooms']
    housing_strat_train['population_per_household'] = housing_strat_train[
        'population'] / housing_strat_train['households']
    #correlations(housing_strat_train)
    ...
    housing_strat_train = strat_train_set.drop(
        'median_house_value', axis=1)  #remove median house value
    housing_strat_train_labels = strat_train_set['median_house_value'].copy(
    )  #use median house value as the target/label for training
    sample_incomplete_rows = housing_strat_train[housing.isnull().any(
        axis=1)].head()
    median = housing['total_bedrooms'].median()
    sample_incomplete_rows['total_bedrooms'].fillna(median, inplace=True)

    imputer = SimpleImputer(strategy='median')
    housing_strat_train_num = housing_strat_train.drop(
        'ocean_proximity', axis=1
    )  #remove text attributes because median can only calculated on numerical attributes
    ...
    housing_strat_train_tr = pd.DataFrame(
        X,
        columns=housing_strat_train_num.columns,
        index=housing_strat_train_num.index)  #tr = transform
    #print(housing_strat_train_tr.head())

    #preprocess the categorical input feature, ocean_proximity
    housing_strat_train_cat = housing_strat_train[[
        'ocean_proximity'
    ]]  #the difference between [] and [[]] is that [[]] remove Name and dtype at the bottom
    #print(f'{housing_strat_train_cat.head(10)}\n')
    ordinal_encoder = OrdinalEncoder()
    housing_strat_train_cat_encoded = ordinal_encoder.fit_transform(
        housing_strat_train_cat)
    ...
    housing_strat_train_cat_onehot = cat_encoder.fit_transform(
        housing_strat_train_cat)
    ...
    num_pipeline = Pipeline([
        ('imputer', SimpleImputer(strategy='median')),
        ('attribs_adder',
         FunctionTransformer(add_extra_features,
                             validate=False,
                             kw_args={
                                 'add_bedrooms_per_room': False,
                                 'housing': housing_strat_train
                             })),
        ('std_scaler', StandardScaler()),
    ])

    housing_strat_train_num_tr = num_pipeline.fit_transform(
        housing_strat_train_num)
    ...
    housing_strat_train_prepared = full_pipeline.fit_transform(
        housing_strat_train)
    ...
    randomizedsearchcv_randomforest(housing_strat_train_prepared,
                                    housing_strat_train_labels, num_attribs,
                                    full_pipeline, strat_test_set)


def randomizedsearchcv_randomforest(housing_prepared, housing_labels,
                                    num_attribs, pipeline, test_set):
    param_distribs = {
...
def gridsearchcv_randomforest(housing_prepared, housing_labels):
    param_grid = [
        ...
        },  #try 12 (3 (n_estimators) * 4 (max_features)) combinations of hyperparameters
        {
            'bootstrap': [False],
            'n_estimators': [3, 10],
            'max_features': [2, 3, 4]
...
def compare_props(housing, strat_test_set, test_set):
    ...
    compare_props['Rand. %error'] = 100 * compare_props[
        'Random'] / compare_props['Overall'] - 100
