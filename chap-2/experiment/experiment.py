...
def main(data):
    ...
    )  # add income_cat to test_set and train_set
    ...
    housing_strat_train["rooms_per_household"] = (
        housing_strat_train["total_rooms"] / housing_strat_train["households"]
    )
    housing_strat_train["bedrooms_per_room"] = (
        housing_strat_train["total_bedrooms"] / housing_strat_train["total_rooms"]
    )
    housing_strat_train["population_per_household"] = (
        housing_strat_train["population"] / housing_strat_train["households"]
    )
    # correlations(housing_strat_train)
    ...
    housing_strat_train_labels = strat_train_set[
        "median_house_value"
    ].copy()  # use median house value as the target/label for training
    ...
    housing_strat_train_tr = pd.DataFrame(
        X, columns=housing_strat_train_num.columns, index=housing_strat_train_num.index
    )  # tr = transform
    ...
    randomizedsearchcv_randomforest(
        housing_strat_train_prepared,
        housing_strat_train_labels,
        num_attribs,
        full_pipeline,
        strat_test_set,
    )
...
def randomizedsearchcv_randomforest(
    housing_prepared, housing_labels, num_attribs, pipeline, test_set
):
    param_distribs = {
